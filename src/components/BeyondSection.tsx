 ...
 * как скорость вращения (см. ROTATION_PERIOD_VH ниже). */
const PIN_HEIGHT_VH = 400
/** Скорость вращения кольца — сначала вдвое замедлили ("замедли вращение
 * в два раза"), затем по следующей просьбе ("увеличь в два раза") вернули
 ...
 * PIN_HEIGHT_VH (см. покадровую сцену «Анимация внутри Beyond (1..5)» в
 * Figma, узел 11144:893/999/1028/1057/970). К концу ENTRANCE_VH кольцо
 * уже полностью на месте — и по позиции, и по центровке, и по scale.
 * Остаток пина, до PIN_HEIGHT_VH, кольцо просто продолжает крутиться на
 * уже финальном месте, без единой дополнительной анимации — это важно,
 * чтобы после того как кольцо "приехало и закончило scale", оно больше
 * никуда само по себе не доезжало (по просьбе пользователя — раньше
 ...
 * жалобе "долго поднимается" сократили до 150 — вышло слишком резко
 * (следующая жалоба, "выезжает очень резко, сделай в два раза плавнее"),
 * вернули обратно к 300, затем по прямой просьбе пользователя
 * установили 200 — компромисс между этими двумя точками. */
const ENTRANCE_VH = 200
...
const CARD_COUNT = 3
/** Было 50 — по жалобе "перелистывание слишком резкое" увеличили до 75. */
const CARD_STEP_VH = 75
...
/** Кусочно-линейная интерполяция между 5 keyframe-точками сцены поверх УЖЕ
 * eased-прогресса (см. `entranceEase` в onUpdate — один smoothstep на весь
 * заезд, а не отдельный smoothstep внутри каждого отрезка, как было
 * раньше). Раньше каждый отрезок сам разгонялся из нулевой скорости и сам
 * же тормозил обратно в ноль к своему концу — на стыке двух отрезков
 * скорость дважды падала почти до нуля, и заезд читался как серия
 * "прыжков"/остановок вместо одного плавного подъёма (баг, на который
 * пожаловался пользователь). Один smoothstep даёт мягкий старт/финиш
 * только на границах ВСЕЙ сцены (progress=0/1), а между кадрами скорость
 * меняется только наклоном (сохраняя неравномерность самой
 * Figma-анимации), без обнуления. Принимает уже готовый eased-прогресс
 * (не сырой), чтобы тот же самый eased можно было переиспользовать и для
 * центровки кольца (см. entranceEase в onUpdate) — обе величины идут
 * СТРОГО одной кривой, без риска рассинхрона. Scale в это уже не входит,
 * см. sampleScale. */
function sampleCarousel(eased: number) {
  const segments = CAROUSEL_KEYFRAMES.length - 1
  const scaled = clamp(eased) * segments
...
export default function BeyondSection() {
  ...
  useEffect(() => {
    ...
    /** Mobile+tablet (< lg): статичный размер кольца (size-[23.125rem] на
     * mobile, md:size-[59rem] на tablet) настолько велик относительно
     * высоты вьюпорта в обеих зонах — и особенно с учётом SCALE_START
     * (2.4364× на первом кадре, см. её объявление), который раздувает
     * рендерящийся размер ещё сильнее, — что даже CAROUSEL_KEYFRAMES[0]
     * .translateY (127.97%, общее число на все брейкпоинты, см. её
     * комментарий) не уводит кольцо целиком за нижний край экрана на
     * первом кадре захода — верхний край остаётся в кадре (жалоба
     * пользователя, сперва про tablet, затем то же самое про mobile:
     * "должна стартовать за нижней границей экрана, а не от её
     * середины"). Изначально чинили только tablet (казалось, что на
     * mobile меньшее кольцо не даёт того же эффекта) — оказалось, что при
     * SCALE_START=2.4364 кольцо всё равно вылезает и там, посчитано
     * аналитически и подтверждено измерением (top кольца при
     * entranceEase=0 — 444.75px внутри 844px вьюпорта). Добавка — в px,
     * поверх centerOffsetPx: считает, сколько ещё не хватает, чтобы top
     * кольца оказался вровень с нижним краем вьюпорта ровно при
     * entranceEase=0, и гасится с той же скоростью, с которой сам
     * keyframe[0] естественно сходит на нет в первом сегменте
     * sampleCarousel (см. kf0Weight в onUpdate) — без отдельной
     * fade-кривой, чтобы не давать лишний "рывок" в темпе движения.
     * Desktop не трогает — там уже видно/подтверждено на референсе, и
     * сама формула (через carousel.offsetHeight/sampleRingScale) —
     * измерение в реальном времени, а не магическое число под конкретный
     * брейкпоинт, так что одна и та же формула одинаково годится что для
     * mobile, что для tablet. */
    const isColumnLayoutRange = () => window.innerWidth < 992
    const measureStartExtraPx = () => {
      if (!isColumnLayoutRange()) return 0
    ...
    let startExtraPx = measureStartExtraPx()

    const onResize = () => {
      centerOffsetPx = measureCenterOffset()
      startExtraPx = measureStartExtraPx()
    }
    ...
    if (reduceMotion()) {
      ...
      entrance.style.transform = `translateY(calc(${start.translateY}% + ${startExtraPx}px)) scale(${sampleRingScale(0)})`
    ...
    const trigger = ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        // Центровка кольца (centerOffsetPx, см. измерение выше) теперь
        // ВПЛЕТЕНА в сам заезд — та же eased-величина, что двигает
        // translateY по CAROUSEL_KEYFRAMES (см. sampleCarousel), домножает
        // и centerOffsetPx, так что обе величины доезжают ОДНОВРЕМЕННО и
        // одной кривой, ровно к концу ENTRANCE_VH. Раньше это было
        // отдельной, более поздней фазой (то стартовавшей вместе с блюром
        // заголовка на PIN_HEIGHT_VH, то — в промежуточном "холостом" окне
        // ENTRANCE_VH..PIN_HEIGHT_VH) — в обоих случаях после того, как
        // кольцо уже "приехало и закончило scale" (конец заезда), оно ещё
        // раз заметно доезжало само по себе, без всякой другой сопутствующей
        // анимации рядом, и читалось как самостоятельный "улёт" в сторону
        // (жалоба пользователя). Теперь после ENTRANCE_VH кольцо уже
        // полностью на месте (и по позиции, и по центровке, и по scale) —
        // дальше, до PIN_HEIGHT_VH, оно только крутится (rotation), без
        // единой дополнительной анимации.
        const entranceEase = smoothstep(entranceProgress)
        ...
        // entranceEase к этому моменту равен 1 (см. выше).
        ...
        // к концу первого сегмента) — гасит startExtraPx в том же темпе,
        // без отдельной fade-кривой (см. её комментарий выше).
        ...
        const extraPx =
          centerOffsetPx * entranceEase + startExtraPx * kf0Weight
                  ...
                  >
                    ...
                    >
                      <img
                        ...
                        alt=""
                        // Без `loading="lazy"` — Beyond приклеена и
                        // скролл-джекнута: браузер узнаёт, что картинка
                        // "рядом с вьюпортом", только когда пользователь
                        // уже докрутил ДО секции и видит крутящееся
                        // кольцо, и тогда все 24 превью (суммарно ~700KB)
                        // начинают грузиться разом — жалоба пользователя
                        // на видимую догрузку. Кольцо всегда раскрывается
                        // целиком (entrance-анимация), частями его не
                        // увидеть, так что откладывать эти картинки как
                        // "может, их и не увидят" смысла нет — грузим
                        // сразу с загрузкой страницы, как остальные
                        // некритичные, но небольшие ассеты.
