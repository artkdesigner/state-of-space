 ...
 * просто синхронизировано с этим уже идущим отъездом.
 *
 * Cliff-title проявляется НЕ после этого окна, а параллельно с ним — см.
 * fadeTrigger ниже, у него тот же старт (wrapTop), что и у riseTrigger.
 * Сверено покадрово с референсным видео (эталонная реализация того же
 * перехода): на кадре, где Location1 ещё занимает весь экран и только
 * начинает скругляться, title уже частично проявлен — то есть fade идёт
 * одновременно с отъездом, а не ждёт, пока Location1 полностью скроется.
 * (Раньше здесь стоял отдельный, более ранний баг — riseTrigger ДО
 * wrapTop, из-за которого Cliff ещё не была приклеена и физически въезжала
 * снизу как обычный doc-flow блок, а title в его окне уже фейдился —
 * визуально ломало сцену. Тот баг чинили переносом riseTrigger на ПОСЛЕ
 * wrapTop; сам факт overlap fade с отъездом — это не баг, а то, как это
 * выглядит на референсе.) */
const RISE_VH = 1
/** Долгота окна fadeTrigger — вместе с RISE_VH выше задаёт полную
 * длительность параллельного окна "Location1 уезжает + Cliff-title
 * проявляется" (см. riseTrigger/fadeTrigger ниже: у обоих start = wrapTop,
 * но у fadeTrigger end на RISE_VH + FADE_VH). Картинки в этом окне не
 * участвуют — см. TITLE_FADE_WINDOW ниже. */
const FADE_VH = 1
...
export default function CliffSection() {
   ...
   *    полностью скрылась за верхним краем экрана.
   * 2) fadeTrigger — тот же старт (wrapTop), что у riseTrigger, но длиннее
   *    (RISE_VH + FADE_VH): Cliff-title проявляется из прозрачности
   *    ПАРАЛЛЕЛЬНО с отъездом Location1, а не после него — см. референсное
   *    видео пользователя, title уже частично виден на кадре, где
   *    Location1 ещё занимает весь экран.
  ...
  return (
    <div
      ...
      style={{
        height: `${(1 + FADE_COMPLETE_VH + IMAGES_START_OFFSET_VH + IMAGES_REVEAL_VH) * 100}dvh`,
        marginTop: '-100dvh',
      }}
    >
      <section
