...
export default function Location1Section({ onBookNow }: Location1SectionProps) {
   ...
   * последовательная прокрутка. `(2 + PIN_VH)` вместо `(1 + PIN_VH)` — тот
   * же приём заморозки: лишний вьюпорт держит Location1 приклеенной (уже
   * полностью раскрытой) ещё на всю дистанцию, пока Cliff (следующий
   * сиблинг, подтянутый вверх своим собственным `margin-top: -100vh`, см.
   * CliffSection.tsx) въезжает снизу и полностью закрывает Location1 — тот
   * же настоящий cover-переход. Никакого JS для самого наезда ни в одну,
   * ни в другую сторону — весь эффект даёт разница между высотой
   * Location1-wrap и margin-top на Cliff-wrap.
   *
