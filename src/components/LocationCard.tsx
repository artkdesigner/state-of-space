...
type LocationCardProps = {
  ...
  /** Клик по Location-step — переход к соответствующему слайду. Опционален:
   * не все места, где рендерится LocationCard, умеют прыгать по индексу
   * (см. Location2Retreat.tsx/Location3Panel.tsx) — без него степы остаются
   * некликабельными, как раньше. */
          ...
          {Array.from({ length: STEP_COUNT }).map((_, i) =>
            onStepClick ? (
              <button
                ...
                className={`Location-step h-px w-9.25 shrink-0 cursor-pointer transition-colors duration-500 ${
                  i === activeIndex ? 'bg-white' : 'bg-white/20 hover:bg-white/50'
                }`}
              />
            ) : (
              <span
