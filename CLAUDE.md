...
- **Только относительные единицы, без исключений.** Все размеры — `rem`, `%`,
  ...
  (`w-[500px]` и т.п. запрещены, писать `w-[31.25rem]`). Это касается и
  «неизменных» по брейкпоинтам величин (например, иконка 100px одного размера
  на всех трёх версиях макета — писать как `size-[6.25rem]`, а не
  `size-[100px]`), не только тех, что растут через `clamp()`. Формула
  `clamp()` из раздела «Флюидная вёрстка» скилла (константа в `px`, min/max в
  `rem`) при этом всё равно переводить в rem целиком — писать
  `clamp(2rem, calc(0.75rem + 5.1vw), 6.875rem)`, не мешать `rem` и голый `px`
  в одном значении. Исключение — `border`-толщина в 1px (браузерный дефолт
  Tailwind-утилиты `border`, не арбитрарное значение) — это hairline, его не
  масштabируют нигде в вебе; если для проекта это неприемлемо — сказать явно.
